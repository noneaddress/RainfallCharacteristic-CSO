...
plt.plot(x * 100, y, marker='o', linestyle='', label='proportion of peak period samples')

loc, scale = norm.fit(data_list)
cdf_fit = norm.cdf(x, loc, scale)
plt.plot(x * 100, cdf_fit, color='r', label='CDF')

plt.xlabel('proportion of peak period(%)')
plt.ylabel('CDF')

plt.legend(loc='lower right')
plt.subplots_adjust(left=0.11, bottom=0.13, right=0.97, top=0.95)
plt.text(x=9, y=0.92, s='A pattern', fontsize=18,
